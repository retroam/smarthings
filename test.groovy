...
def on_event(evt)
{
    ...
    def jd = device(dt.device, dt.type)
...
{  
    log.debug "evt ${evt}"
    //def dtd = dtd()
    log.debug "devices_for_type${ dtd()}" 
    for (dt in dtd()) {
        if (dt.key != evt.name) {
        	continue
        }
...
def send(device, device_type, deviced) {
    ...
    def uri = "http://52.0.69.238:3000/users"
    def headers = [:]
    def body = [
        "topic": topic,
        "payload": device,
    ...
    httpPostJson(params) { log.debug "send: response=${response}" }
}
